...
export async function POST(request: NextRequest) {
  try {
    ...
    if (primaryFont.toLowerCase() === 'cairo') primaryFont = 'Cairo';
    ...
    const transformation = [
      ...
      {
        // FIX 1 & 2: Structural split. Subtitle reference via public_id prefix. 
        overlay: {
          ...
          public_id: `subtitles:${vttUpload.public_id}` // FIX 1: ID prefix instead of resource_type
        },
        color: subtitleColor,
      ...
      {
        // FIX 2: Layout and flags split from definition block
        gravity: 'south',
        y: Math.round(40 * 3.5),
        // FIX 3: Force text_shaping to connect Arabic letters (ن , م)
